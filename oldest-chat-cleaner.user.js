...
(() => {
  ...
  deleteButton.addEventListener("click", async () => {
    ...
    const workspace =
      contextSelect.selectedOptions[0]?.textContent || "current workspace";
    if (
      !confirm(
        `Permanently delete the ${targets.length} ${adapter.platform} chats currently shown?\n` +
          `Group: ${activeGroup.name}\n` +
          `Workspace: ${workspace}\n\nPinned and unknown-pin-status chats are protected. This cannot be undone.`,
      )
    )
      return;
    busy = true;
